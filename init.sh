#!/bin/sh
DEMO="Insurance Pricing DMN Demo"
AUTHORS="Red Hat"
PROJECT="git@github.com:jbossdemocentral/rhdm7-insurance-pricing-dmn-demo.git"
...
VERSION=7.0
PROJECT_GIT_REPO=https://github.com/jbossdemocentral/rhdm7-insurance-pricing-dmn-demo-repo
...
then
  ...
  echo
  rm -rf ./target/temp && git clone --bare $PROJECT_GIT_REPO ./target/temp/rhdm7-insurance-pricing-dmn-demo-repo.git || { echo; echo >&2 "Error cloning the project's Git repo. If there is no Internet connection available, please run this script in 'offline-mode' ('-o') to use a previously downloaded and cached version of the project's Git repo... Aborting"; echo; exit 1; }

  echo "  - replacing cached project git repo: $PROJECT_GIT_DIR/rhdm7-insurance-pricing-dmn-demo-repo.git"
  echo
  rm -rf $PROJECT_GIT_DIR/rhdm7-insurance-pricing-dmn-demo-repo.git && mkdir -p $PROJECT_GIT_DIR && cp -R target/temp/rhdm7-insurance-pricing-dmn-demo-repo.git $PROJECT_GIT_DIR/rhdm7-insurance-pricing-dmn-demo-repo.git && rm -rf ./target/temp
else
...
rm -rf $JBOSS_HOME/bin/.niogit/myrepo.git && cp -R $PROJECT_GIT_DIR/rhdm7-insurance-pricing-dmn-demo-repo.git $SERVER_BIN/.niogit/myrepo.git
