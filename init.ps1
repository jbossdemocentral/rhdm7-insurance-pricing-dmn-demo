...
If (! $o) {
  ...
  $argList = "clone --bare $PROJECT_GIT_REPO $PROJECT_HOME\target\temp\rhdm7-insurance-pricing-dmn-demo-repo.git"
  ...
  Write-Host ""
  Write-Host "  - replacing cached project git repo: $PROJECT_GIT_DIR/rhdm7-insurance-pricing-dmn-demo-repo.git`n"
  If (Test-Path "$PROJECT_GIT_DIR") {
	Remove-Item "$PROJECT_GIT_DIR" -Force -Recurse
  }
  New-Item -ItemType directory -Path "$PROJECT_GIT_DIR"
  Copy-Item "$PROJECT_HOME\target\temp\rhdm7-insurance-pricing-dmn-demo-repo.git" "$PROJECT_GIT_DIR\rhdm7-insurance-pricing-dmn-demo-repo.git" -Force -Recurse
  Remove-Item "$PROJECT_HOME\target\temp" -Force -Recurse
} else {
  Write-Host "  - running in offline-mode, using cached project's Git repo.`n"

  If (-Not (Test-Path "$PROJECT_GIT_DIR\rhdm7-insurance-pricing-dmn-demo-repo.git")) {
...
Copy-Item "$PROJECT_GIT_DIR\rhdm7-insurance-pricing-dmn-demo-repo.git" "$SERVER_BIN\.niogit\myrepo.git" -force -recurse
